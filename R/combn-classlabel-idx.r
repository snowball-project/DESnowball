...
#    if(length(leave.k)!=nlevels(classlabel) & length(leave.k) != 1)
	...
	for (i in seq(length(classlabelUnique))) {
		...
		if(length(leave.k) == 1)
		    m <- floor(sum(classlabel==classlabelUnique[i])*leave.k)
		else m <- floor(sum(classlabel==classlabelUnique[i])*leave.k[i])
	    }
	    else m <- leave.k[i]
	    ret[[i]] <- combn(which(classlabel==classlabelUnique[i]),m)
	}
    }
